...
func (a *DevsyWorkspaceTemplateVersion) GetVersion() string {
	return a.Version
}

func (a *DevsyWorkspaceTemplate) GetOwner() *UserOrTeam {
	return a.Spec.Owner
}

func (a *DevsyWorkspaceTemplate) SetOwner(userOrTeam *UserOrTeam) {
	a.Spec.Owner = userOrTeam
}

func (a *DevsyWorkspaceTemplate) GetAccess() []Access {
	return a.Spec.Access
}

func (a *DevsyWorkspaceTemplate) SetAccess(access []Access) {
...
type DevsyWorkspaceTemplateSpec struct {
	...
	// Template holds the Devsy workspace template
	Template DevsyWorkspaceTemplateDefinition `json:"template,omitempty"`

	// Versions are different versions of the template that can be referenced as well
	// +optional
	Versions []DevsyWorkspaceTemplateVersion `json:"versions,omitempty"`
...
type DevsyWorkspaceTemplateDefinition struct {
	...
	// +optional
	WorkspaceEnv map[string]DevsyProviderOption `json:"workspaceEnv,omitempty"`

	// InstanceTemplate holds the workspace instance template
	// +optional
	InstanceTemplate DevsyWorkspaceInstanceTemplateDefinition `json:"instanceTemplate,omitempty"`
	...
	//
	// +optional
	Provider *DevsyWorkspaceProvider `json:"provider,omitempty"`
}

type DevsyWorkspaceKubernetesSpec struct {
	// Pod holds the definition for workspace pod.
	//
	// Defaults will be applied for fields that aren't specified.
	// +optional
	Pod *DevsyWorkspacePodTemplate `json:"pod,omitempty"`
	...
	// Defaults will be applied for fields that aren't specified.
	// +optional
	VolumeClaim *DevsyWorkspaceVolumeClaimTemplate `json:"volumeClaim,omitempty"`
	...
	// SpaceTemplateRef is a reference to the space that should get created for this Devsy.
	...
	// SpaceTemplate is the inline template for a space that should get created for this Devsy.
	// If this is specified, the kubernetes provider will be selected automatically.
	// +optional
	SpaceTemplate *SpaceTemplateDefinition `json:"spaceTemplate,omitempty"`

	// VirtualClusterTemplateRef is a reference to the virtual cluster that should get created for this Devsy.
	// If this is specified, the kubernetes provider will be selected automatically.
	// +optional
	VirtualClusterTemplateRef *TemplateRef `json:"virtualClusterTemplateRef,omitempty"`

	// VirtualClusterTemplate is the inline template for a virtual cluster that should get created for this Devsy.
	// If this is specified, the kubernetes provider will be selected automatically.
	// +optional
...
type DevsyWorkspacePodTemplate struct {
	...
	Spec DevsyWorkspacePodTemplateSpec `json:"spec,omitempty"`
}

// DevsyWorkspacePodResourceRequirements are less restrictive corev1.ResourceRequirements.
type DevsyWorkspaceResourceRequirements struct {
...
type DevsyWorkspaceContainer struct {
	...
	// +optional
	Resources DevsyWorkspaceResourceRequirements `json:"resources,omitempty"`
...
type DevsyWorkspacePodTemplateSpec struct {
	...
	// +optional
	InitContainers []DevsyWorkspaceContainer `json:"initContainers,omitempty"`

	// List of containers belonging to the pod.
	// +optional
	Containers []DevsyWorkspaceContainer `json:"containers,omitempty"`
...
type DevsyWorkspaceVolumeClaimTemplate struct {
	// The pods metadata
	// +kubebuilder:pruning:PreserveUnknownFields
	// +optional
	TemplateMetadata `json:"metadata,omitempty"`

	Spec DevsyWorkspaceVolumeClaimSpec `json:"spec,omitempty"`
}

type DevsyWorkspaceVolumeClaimSpec struct {
...
type DevsyWorkspaceProvider struct {
	...
	// +optional
	Options map[string]DevsyProviderOption `json:"options,omitempty"`

	// Env are environment options to set when using the provider.
	// +optional
	Env map[string]DevsyProviderOption `json:"env,omitempty"`
}

type DevsyWorkspaceInstanceTemplateDefinition struct {
...
type DevsyProviderOption struct {
	...
	// +optional
	ValueFrom *DevsyProviderOptionFrom `json:"valueFrom,omitempty"`
}

type DevsyProviderOptionFrom struct {
...
type DevsyProviderSource struct {
...
type DevsyWorkspaceTemplateVersion struct {
	// Template holds the Devsy template
	// +optional
	Template DevsyWorkspaceTemplateDefinition `json:"template,omitempty"`
...
func init() {
	SchemeBuilder.Register(&DevsyWorkspaceTemplate{}, &DevsyWorkspaceTemplateList{})
}
